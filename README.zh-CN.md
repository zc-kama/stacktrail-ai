...
StackTrail 是一套可迁移的 AI 项目开发工作流。它让 AI 编程助手不只是“凭空写代码”，而是先读项目记忆，顺着前端、后端、数据库、配置、部署等链路理解现有系统，再做最小完整改动、验证受影响层级，复盘重要错误，并把长期有用的项目事实和错误教训写回项目记忆。
...
StackTrail 的目标不是增加流程负担，而是让 AI 在动手前先知道“数据从哪里来，到哪里去，谁在消费它”，出错后也知道“为什么错，怎么避免下次再错”。
...
8. 复盘重要错误、错误假设、失败命令和回归问题。
9. 如果产生了长期事实或可复用错误教训，就更新项目记忆。
...
- 出错内容不能只修完就忘，要提炼成未来能避坑的规则。
- 用户在学习时，从当前文件、当前页面或当前错误讲起，再逐步展开。

## 错误复盘

StackTrail 要求把有价值的错误也写进项目记忆。
...
- 部署问题没有先查证据就猜配置
- 验证时发现回归
- 旧构建产物、旧配置、隐藏兼容规则导致误判

推荐记录格式：

```text
现象：什么失败了，或者看起来哪里不对
原因：错误假设是什么，缺失的项目事实是什么
修复：最终怎么解决
规则：未来再遇到类似问题时应该先检查什么
```

不要把完整日志、完整报错和聊天记录都塞进项目记忆。只保留未来开发真的能复用的教训。
...
- 日志、缓存、会话、权限规则
- 可重复的排查经验
- 重要错误、错误假设、失败修复和回归问题暴露出的项目规则

不要把完整聊天记录塞进项目记忆。只记录未来开发真的会用到的事实和教训。
